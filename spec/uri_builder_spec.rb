...
describe RepoForker::URI_Builder do
  ...
  describe ".build" do
    it "returns a Ruby URI::HTTPS object" do
      expect(RepoForker::URI_Builder.build fake_repo_to_fork).to be_an_instance_of URI::HTTPS
    end

    it "has a path to request forking the repository" do
      expected_path = "/repos/#{fake_source}/#{fake_repo_to_fork}/forks"
      built_uri = RepoForker::URI_Builder.build(fake_repo_to_fork)
